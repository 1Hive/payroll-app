...
contract('Payroll roles', () => {
  ...
  describe('roles', () => {
    it('should implement role constants successfully', async () => {
      ...
      assert.equal(await payroll.SET_EQUITY_MULTIPLIER_ROLE(), web3.utils.sha3('SET_EQUITY_MULTIPLIER_ROLE'), 'set equity role does not match')
      assert.equal(await payroll.SET_VESTING_SETTINGS_ROLE(), web3.utils.sha3('SET_VESTING_SETTINGS_ROLE'), 'set vesting settings role does not match')
    })
  })
})
