...
contract('Payroll initialization', ([owner]) => {
  ...
  describe('initialize', function () {
    ...
    it('cannot initialize the base app', async () => {
      ...
      await assertRevert(payrollBase.initialize(finance.address, DAI.address, equityTokenManager.address, 1, 0, 0, false, { from: owner }), 'INIT_ALREADY_INITIALIZED')
    })

    context('when it has not been initialized yet', function () {
      it('reverts when passing an invalid finance instance', async () => {
        await assertRevert(payroll.initialize(ZERO_ADDRESS, DAI.address, equityTokenManager.address, 1, 0, 0, false, { from: owner }), 'PAYROLL_FINANCE_NOT_CONTRACT')
      })

      it('reverts when initialized with a non-contract denomination token', async () => {
        await assertRevert(payroll.initialize(finance.address, ZERO_ADDRESS, equityTokenManager.address, 1, 0, 0, false, { from: owner }), 'PAYROLL_DENOMINATION_TOKEN_NOT_CONTRACT')
      })

      it('reverts when initialized with a non-contract token manager', async () => {
        await assertRevert(payroll.initialize(finance.address, DAI.address, ZERO_ADDRESS, 1, 0, 0, false, { from: owner }), 'PAYROLL_TOKEN_MANAGER_NOT_CONTRACT')
      })

      it('reverts when initialized with bad vesting settings', async () => {
        await assertRevert(payroll.initialize(finance.address, DAI.address, equityTokenManager.address, 1, 10, 11, false, { from: owner }), 'PAYROLL_CLIFF_PERIOD_TOO_HIGH')
      })
    })
    ...
    context('when it has already been initialized', function () {
      ...
      it('cannot be initialized again', async () => {
        await assertRevert(payroll.initialize(finance.address, DAI.address, equityTokenManager.address, 1, 0, 0, false, { from: owner }), 'INIT_ALREADY_INITIALIZED')
      })
      ...
      it('has a price feed instance, a finance instance, a denomination token and a rate expiration time', async () => {
        ...
        assert.equal(await payroll.equityMultiplier(), 1, 'equityMultiplier does not match')
